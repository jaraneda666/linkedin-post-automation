...
| `400 Bad Request` en "Generar Imagen con Texto - Pollinations+Cloudinary" con `x-cld-error: public_id ... is too long` | El `imagePrompt` (estilo + escena) es demasiado largo; al codificarse dos veces (URL de Pollinations dentro de la URL de fetch de Cloudinary) supera el límite de longitud del `public_id` remoto de Cloudinary (~150-155 caracteres sin codificar, verificado empíricamente) | Mantener el `imagePrompt` combinado (escena + `baseStyle`) por debajo de ~130 caracteres. Si agregas más detalle de estilo, acórtalo en otro lado para no pasar el límite |
| `getaddrinfo ENOTFOUND api-inference.huggingface.co` | HuggingFace deprecó ese endpoint (dejó de resolver desde fines de 2025 / confirmado marzo 2026) | Usar `https://router.huggingface.co/hf-inference/models/<modelo>` en su lugar |
